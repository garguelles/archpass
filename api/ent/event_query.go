...
func (eq *EventQuery) QueryAttendees() *AttendeeQuery {
	query := (&AttendeeClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EventQuery) WithAttendees(opts ...func(*AttendeeQuery)) *EventQuery {
	query := (&AttendeeClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (eq *EventQuery) loadTickets(ctx context.Context, query *TicketQuery, nodes []*Event, init func(*Event), assign func(*Event, *Ticket)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(ticket.FieldEventID)
	}
...
func (eq *EventQuery) loadAttendees(ctx context.Context, query *AttendeeQuery, nodes []*Event, init func(*Event), assign func(*Event, *Attendee)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Event)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(attendee.FieldEventID)
	}
